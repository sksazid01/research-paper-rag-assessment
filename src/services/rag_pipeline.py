...
"""
RAG Pipeline — the heart of this system.

Roles:
1. Query → Embedding → Qdrant retrieval  (find the most relevant chunks)
2. Re-rank those chunks with a cross‑encoder (for precision)
3. Build context-aware prompt
4. Send prompt to Ollama (LLM) → generate final answer
5. Extract citations and estimate confidence

This module unifies everything from your `services`: qdrant_client, embedding_service, and ollama_client.
"""
...
def get_cross_encoder():
    ...
    The cross‑encoder is a second model used to refine relevance.
    It takes (query, passage) pairs and predicts how well they match.
    Lazy loading ensures it’s loaded only when first needed (saves memory).
    """
...
def get_paper_info(paper_id: int) -> Optional[Dict]:
    """Fetch paper metadata from postgre database."""
...
def retrieve_context(
    query: str, top_k: int = 5, paper_ids: Optional[List[int]] = None
) -> List[Dict]:
    ...
    if paper_ids:
        ...
        """
		If a question references certain papers, restrict retrieval to those IDs.
        Otherwise, search globally across all stored vectors.
        """
        query_filter = models.Filter(
            should=[
                models.FieldCondition(
                    key="paper_id", match=models.MatchValue(value=pid)
                )
    ...
    SCORE_THRESHOLD_FILTERED = float(
        os.getenv("RETRIEVAL_SCORE_THRESHOLD_FILTERED", "0.05")
    )
    SCORE_THRESHOLD = SCORE_THRESHOLD_FILTERED if paper_ids else SCORE_THRESHOLD_DEFAULT
    hits = qdrant_client.search(
        vec, limit=top_k, query_filter=query_filter, score_threshold=SCORE_THRESHOLD
    )
    ...
    try:
        if not hits:
            print(
                f"[DEBUG] retrieve_context: no hits returned for query (top_k={top_k}, threshold={SCORE_THRESHOLD})"
            )
        else:
            scores = [round(h.score, 4) for h in hits]
            print(
                f"[DEBUG] retrieve_context: returned {len(hits)} hits, scores={scores}"
            )
    except Exception:
        ...
        contexts.append(
            {
                ...
                "paper_id": paper_id,
                "paper_title": (
                    paper_info.get("title") if paper_info else f"Paper {paper_id}"
                ),
	...
	# If the question contains domain words like “transformer” or “attention,” chunks containing these terms receive a small bonus to their scores. This improves recall for domain‑specific queries.
    ql = (query or "").lower()
    boost_terms = [
        t
        for t in ["attention", "transformer", "transformers", "nlp", "neural network"]
        if t in ql
    ]
    if boost_terms:
        def boosted(c):
...
def rerank_contexts(query: str, contexts: List[Dict], top_k: int = None) -> List[Dict]:
    ...
    try:
        ...
        print(
            f"[WARNING] Cross-encoder re-ranking failed: {e}. Falling back to original order."
        )
        return contexts
...
def calculate_confidence(contexts: List[Dict], answer_text: str) -> float:
    ...
    avg_score = (
        sum(c.get("score", 0) * (1.0 / (i + 1)) for i, c in enumerate(contexts[:3]))
        / 3.0
    )
    ...
    has_uncertainty = any(
        phrase in answer_text.lower() for phrase in uncertainty_phrases
    )
...
def answer(
    ...
    if len(q_lower) < 5 or any(
        pattern == q_lower or q_lower.startswith(pattern + " ")
    ...
    sources_used = list(
        set(c.get("paper_filename") for c in contexts if c.get("paper_filename"))
    )
    paper_ids_used = list(
        {c.get("paper_id") for c in contexts if c.get("paper_id") is not None}
    )
