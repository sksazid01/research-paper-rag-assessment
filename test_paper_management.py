...
def test_list_papers():
    ...
    if response.status_code == 200:
        ...
        for paper in result['papers']:
            ...
            print(f"  Authors: {paper['authors'][:50]}..." if paper['authors'] and len(paper['authors']) > 50 else f"  Authors: {paper['authors']}")
...
def test_delete_paper(paper_id):
    """Test DELETE /api/papers/{id} - Delete paper (use with caution!)."""
    print("\n" + "="*60)
    print(f"Test 4: Delete Paper (ID: {paper_id})")
    print("="*60)
    
    confirm = input(f"\n⚠️  WARNING: This will permanently delete paper {paper_id}. Continue? (yes/no): ")
    
    if confirm.lower() != 'yes':
        print("Deletion cancelled.")
        return
    
    response = requests.delete(f"{API_BASE}/papers/{paper_id}")
    
    if response.status_code == 200:
        result = response.json()
        print(f"\n✅ {result['message']}")
        print(f"Paper: {result.get('paper_title')}")
        print(f"Vectors Deleted: {result.get('vectors_deleted')}")
    elif response.status_code == 404:
        print(f"\n⚠️  Paper not found")
    else:
        print(f"\n❌ Failed: {response.status_code}")
...
def test_get_nonexistent_paper():
    ...
    print("\n" + "="*60)
    print("Test 5: Get Non-existent Paper (Error Handling)")
    print("="*60)
...
if __name__ == "__main__":
    try:
        ...
        if papers:
            # Test get specific paper (use first paper ID)
            ...
            # test_delete_paper(first_paper_id)
